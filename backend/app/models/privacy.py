...
class ConsentRecord(UuidPrimaryKeyMixin, TimestampMixin, Base):
    ...
    type: Mapped[ConsentType] = mapped_column(enum_type(ConsentType), nullable=False)
    status: Mapped[ConsentStatus] = mapped_column(enum_type(ConsentStatus), nullable=False)
    captured_at: Mapped[datetime] = mapped_column(nullable=False)
    evidence_type: Mapped[ConsentEvidenceType] = mapped_column(
        enum_type(ConsentEvidenceType),
        nullable=False,
    )
...
class RetentionPolicy(UuidPrimaryKeyMixin, TimestampMixin, Base):
    ...
    delete_mode: Mapped[DeleteMode] = mapped_column(enum_type(DeleteMode), nullable=False)
...
class AuditLog(UuidPrimaryKeyMixin, Base):
    ...
    actor_type: Mapped[AuditActorType] = mapped_column(enum_type(AuditActorType), nullable=False)
